...
void calib(int image_count, Size board_size) {
    ...
    vector<vector<Point2f>>  corners_Seq;                     //     ?????????????§ß??
    ...
    ??????????
    *************************************************************************/
    vector<vector<Point3f>>  object_Points;        /****  ???ÕÇ?????????????????   ****/

    Mat image_points = Mat(1, count, CV_32FC2, Scalar::all(0));  /*****   ????????????§ß??   *****/
...
int dist2line(Point pt, Point p1, Point p2) {	//?§Ø?????????????????
    int A = p1.y - p2.y;
    int B = p2.x - p1.x;
