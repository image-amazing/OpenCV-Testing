// FisheyeCalib.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	for (int i = 0; i != image_count; i++)
	{
		...
		imageFileName += ".jpg";
		cv::Mat image = imread("VMRImage" + imageFileName);
	...
	*************************************************************************/
	...
	newCameraMatrix.at<float>(0, 0) = intrinsic_matrix.val[0];
	newCameraMatrix.at<float>(0, 2) = 1.0*intrinsic_matrix.val[2];
	newCameraMatrix.at<float>(1, 1) = intrinsic_matrix.val[4];
	newCameraMatrix.at<float>(1, 2) = 1.0*intrinsic_matrix.val[5];
	cout << intrinsic_matrix << endl;
	cout << newCameraMatrix << endl;
	Size new_image_size = Size(1.0 * image_size.width, 1.0 * image_size.height);
	cout << "保存矫正图像" << endl;
	for (int i = 0; i != image_count; i++)
	{
		cout << "Frame #" << i + 1 << "..." << endl;
		fisheye::initUndistortRectifyMap(intrinsic_matrix, distortion_coeffs, R, newCameraMatrix, new_image_size, CV_32FC1, mapx, mapy);
